...
def run(inputs, outputs):
    ...
    while True:
        ...
        if instruction.code == 1:
            ...
            outputs.append(f"{instruction.get_param(1, ipx)}")
            ipx += 2
...
def valid_setting(setting_num):
    options = set(str("{:05d}".format(setting_num)))
    return len(options) == 5 and len(options.intersection(["5", "6", "7", "8", "9"])) == 0


possible_phase_settings = [list("{:05d}".format(n)) for n in range(50000) if valid_setting(n)]
highest_output = 0
...
for phase_setting in possible_phase_settings:
    ...
    print()
    for amp in range(5):
        program = amplifier_software.copy()
        print([phase_setting[amp], input_signal])
